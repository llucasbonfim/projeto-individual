...
select count(fkmapa) 'votos', nome_mapa 'mapa' from usuario join mapa on idmapa = fkmapa group by nome_mapa;
select count(fkarma) 'votos', nome_arma 'arma' from usuario join arma on idarma = fkarma group by nome_arma;
select count(fkoperador) 'votos', nome_operador 'operador' from usuario join operador on idoperador = fkoperador group by nome_operador;
